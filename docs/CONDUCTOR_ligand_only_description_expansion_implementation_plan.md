...
この文書は、`cs-conductor-desc-ligand` Skill におけるLigand単体Descriptionの追加実装計画と現行実装仕様である。合意事項、追加したdescriptor/fingerprint/embedding候補、config方針、テスト方針を固定する。
...
- 実装済み: Hugging Face local model adapter for compatible SMILES/SELFIES transformer models
- 実装済み: `local_custom` adapter contract for model-dir bundled adapters
- 実装済み: `external_command` adapter contract for repo-native embedding CLIs
- 実装済み: `unimol_local` adapter path using `unimol_tools`
- 実装済み: `mol2vec_local` adapter path using `mol2vec` + `gensim`
...
- 未実測: ChemBERTa以外のpretrained embeddingはlocal weight未提供のためJAK実測未実施
...
- L30 ChemBERTa-5M-MLMはlocal weightでJAK 231化合物のCSV出力成功、384次元、エラー0
- L30 ChemBERTa-10M-MLMはlocal weightでJAK 231化合物のCSV出力成功、384次元、エラー0
- L33はadapter実装済み。`unimol_tools` とlocal model_dirが未提供のためJAK実測は未実施
...
Tier 1、Tier 2、ユーザ承認済み追加対象は、local weight pathまたはmodel固有adapterが与えられたときに実行できるadapter経路を実装対象とする。残りTier 3は候補として保持するが、現時点では実装対象外とする。古いRNN/LSTM系は、代表モデル名とgeneric adapter方針を残すに留める。

### 8.3 Adapter契約とWeight管理

この段階ではweight downloadは行わない。実行時はconfigに明示されたlocal pathだけを読む。
...
- 出力CSVにはID/metadata共通列とnumeric embedding列だけを入れる。
- model名、checkpoint path、pooling、tokenizer、入力正規化方法は `run_metadata.json` の `descriptor_metadata` に残す。
...
- `mol_parse_ok`
- `descriptor_error`
- `embedding__dim_0000` 以降のnumeric embedding columns

`local_custom` は `model_dir` 配下の `conductor_embedding_adapter.py` または `adapter.py` を読む。adapterは `embed_molecules(records, model_dir, params)` または `embed_smiles(smiles, model_dir, params)` を実装し、2D vector、`DataFrame`、または `{vectors, metadata}` を返す。

`external_command` は `params.command` に `{input_csv}`, `{output_csv}`, `{model_dir}`, `{work_dir}` placeholderを持つcommandを設定する。commandは `compound_id` とnumeric feature列を持つCSVを `{output_csv}` に書く。
...
Phase C: 量子化学 コード完了、現Windows `.venv` で依存未導入

10. L60 tblite xTB descriptorを実装する。
11. `tblite-python` が利用可能な環境でtblite smoke test、JAK subset test、可能ならJAK全件testを行う。

Phase D: foundation model embedding adapter経路完了、ChemBERTaのみlocal weight実測完了

12. L30 ChemBERTa adapterを実装する。
13. L31 MolFormer adapterを実装する。
14. L33 Uni-Mol adapterを実装する。
15. L32 GROVER external command adapter経路を実装する。
16. L34-L36 IBM materials系、L37-L39 sequence系、L40-L43、L44-L49のadapter経路を実装する。

残作業:

- ChemBERTa以外のlocal weightを受け取った時点で、該当setごとにJAK testを実施する。
- `tblite-python` が導入できる環境でL60をJAK subset以上で確認する。
