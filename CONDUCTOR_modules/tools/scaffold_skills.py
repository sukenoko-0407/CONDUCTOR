...
- CONDUCTORではInterpreterがこのSkillでdraftを事前検査できる。正式ID、scope、Markdown／HTML、Ledger commitは現行Runtimeだけが確定する。'''
...
def create_skill(capability: dict[str, Any], kind: str, template: Path, schemas: list[str], force: bool) -> bool:
    ...
    if kind in {"description", "clustering", "analysis"}:
        if kind == "description":
            adapter, roles = "description", ["dataset"]
        elif kind == "clustering":
            family = capability.get("family")
            algorithm = capability.get("implementation", {}).get("algorithm")
            adapter = "categorical_clustering" if algorithm == "categorical" else "vector_clustering" if family == "description_vector" else "meta_clustering" if family == "meta" else "structure_clustering"
            roles = ["description"] if adapter == "vector_clustering" else ["cluster_membership_matrix"] if adapter == "meta_clustering" else ["dataset"]
        elif capability["capability_id"] in {"A003", "A004"}:
            adapter, roles = "projection_operator", ["dataset"]
        elif capability["capability_id"] == "A005":
            adapter, roles = "multidescription_operator", ["dataset", "description"]
        else:
            adapter, roles = "standard_operator", ["dataset"]
        capability["conductor_request"] = {"schema_version": "1.0.0", "adapter": adapter, "required_input_roles": roles, "unknown_parameters": "reject_by_skill_cli"}
    ...
    shutil.copy2(TEMPLATES / "launch.py", skill_dir / "scripts" / "launch.py")
    if kind in {"description", "clustering", "analysis"}:
        shutil.copy2(TEMPLATES / "conductor_request_adapter.py", skill_dir / "scripts" / "conductor_request_adapter.py")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create or refresh CONDUCTOR Skill folders from canonical templates.")
