...
def descriptor_correlation(description: pd.DataFrame | None, features: list[str]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    top = result.iloc[0].to_dict() if len(result) else {}
    signed = top.get("spearman_rho") if top else None
    return result, {
        "tested_feature_count": len(result),
        "top_features": result.head(10)["feature"].tolist() if len(result) else [],
        "top_feature": top.get("feature"),
        "max_abs_association": top.get("max_abs_association"),
        "top_signed_association": signed,
        "top_association_pvalue": top.get("spearman_pvalue"),
    }
...
def run() -> int:
    ...
    manifest = {"schema_version": "2.0.0", "conductor_version": "0.1.8", "artifact_stage": "analysis", "run_id": run_id, "node_id": args.node_id, "attempt_id": args.attempt_id, "capability_id": CAPABILITY["capability_id"], "operator_id": CAPABILITY["operator_id"], "skill_name": CAPABILITY["skill_name"], "skill_version": CAPABILITY["version"], "input": args.input, "input_hash": input_hash, "value_semantics": "operator_result", "natural_metric": summary.get("metric"), "id_column": id_column, "property_column": args.property_column, "higher_is_better": args.higher_is_better, "description": args.description, "membership": args.membership, "scope": scope, "output": result_path.name, "warnings": warnings, "created_at": utc_now(), "configuration": config}
    if args.conductor:
