...
def _key_metrics(value: dict[str, Any]) -> str:
    metrics = value.get("key_metrics") or {}
    parts = []
    for result_ref, result_metrics in metrics.items():
        formatted = ", ".join(f"{key}={number}" for key, number in result_metrics.items()) or "主要数値なし"
        parts.append(f"{result_ref}: {formatted}")
...
def render_markdown(report: dict[str, Any], report_dir: Path | None = None, run_root: Path | None = None) -> str:
    ...
    lines = [f"# {report['title']}", "", f"- Run / Round: `{report['run_id']}` / `{report['round_id']}`", f"- Endpoint: `{header['endpoint']}` (`higher_is_better={str(header['higher_is_better']).lower()}`)", f"- 単位 / 変換: {header.get('endpoint_unit') or '未指定'} / {header.get('endpoint_transform') or 'なし'}", f"- Round Outcome: `{header['completion']}`", "", "## エグゼクティブサマリー", "", report["executive_summary"], "", "## 解析範囲", "", report["coverage_summary"], "", f"- 詳細確認: {len(review['detailed_result_refs'])}", f"- 集約確認: {len(review['aggregate_result_refs'])}", f"- 未確認: {len(review['unreviewed_results'])}", f"- Scope別: {json.dumps(review['scope_counts'], ensure_ascii=False)}", f"- Operator別: {json.dumps(review['operator_counts'], ensure_ascii=False)}", "", "## Insight", ""]
    ...
    lines += ["", "## 未確認範囲", ""]
    lines += [f"- {item.get('result_ref')}: {item.get('reason')}" for item in review["unreviewed_results"]] or ["- なし"]
    return "\n".join(lines) + "\n"
