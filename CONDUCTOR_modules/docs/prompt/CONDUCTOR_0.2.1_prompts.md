...
- 0.1.10/0.1.11 の Description Database は、通常は変換せずその場で再利用する。既定配置は `<PROJECT_ROOT>/data/description_database/<PROGRAM_NAME>/` とする。人間が全件再計算を明示決定し、この既定pathにDatabaseが存在しない場合だけ、3.2Aと3.4Aの例外経路を使う。
...
### 3.2A 全Description新規構築前のPreflight
...
人間が全Descriptionの再計算を明示決定しています。次を確認してください。
- 必須列、IDの欠損・空文字・重複
- SMILESの空欄、RDKit parse不能数
- Endpointのtransform domain、finite件数、欠測数
- Run rootが新規かつ空であること
