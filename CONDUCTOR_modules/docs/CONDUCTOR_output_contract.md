# CONDUCTOR 0.1.8 output contract
...
`screening_summary.json`はBundle件数、Candidate class、Operator構成、掲載候補pointerだけを持つcompact handoffです。`report_mode=screening`ではこれが正式Interpretationの代わりとなり、`report_mode=full`では`favorable_clue`と`contextual_clue`から最大50 Resultを選抜します。
