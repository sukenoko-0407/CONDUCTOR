...
対象Versionは`0.1.8`です。Description、Clustering、Operator、Interpretationを疎結合に接続し、同じRunを人間主導の複数Roundで探索します。共通Execution Request、Global優先の単一exploration、Review Bundleによる絶対複数軸評価、活性改善候補中心のInterpretation、実務的範囲の省容量MMP Databaseを採用します。
...
- [CONDUCTOR_assessment_guide.md](CONDUCTOR_assessment_guide.md): 一次評価軸、Candidate class、対象条件、SummaryとFull Reportの選抜差
- [CONDUCTOR_error_recovery_and_assessment_refactor_proposal.md](CONDUCTOR_error_recovery_and_assessment_refactor_proposal.md): エラー回復と一次評価再設計の提案、MMPのSpotfire利用方針
...
- [CONDUCTOR_0.1.8_result_interpretation_refactor_proposal.md](CONDUCTOR_0.1.8_result_interpretation_refactor_proposal.md): Result Card、Review Bundle、絶対評価、Interpretation再設計
- [CONDUCTOR_version_history_0.1.8.md](CONDUCTOR_version_history_0.1.8.md): 0.1.8の実装変更と互換性
...
`prompt/`には初回、探索screening Round、full Interpretation Round、継続Round、複数の完了済みScreening Roundを対象にした累積Interpretation、別session引継ぎ、Failed Nodeの同一Round再試行、現在RoundおよびCLOSED Roundの一次評価再Screening、read-only MMP Global–Local解釈、一次評価サマリー、Concierge用のプロンプト例があります。Catalog収載対象は`catalog/included_skills.json`、標準解析範囲は`catalog/analysis_profile.json`が正本です。0.1.8は旧Runを継続せず、新規Runとして開始します。
