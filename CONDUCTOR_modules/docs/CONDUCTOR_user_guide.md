...
parallel limitは同時に実行するNode数、Available CPU Coresは科学計算へ割り当てるCPU総数です。C002（MCS）とD016（Mordred 3D）は最大8個の単一thread worker、D019（GFN2-xTB）は原則として1化合物4コア、`floor(Available CPU Cores / 4)`化合物並列で、いずれもRuntimeにより単独実行されます。D020（ChemBERTa）とA014 Global MMPも他Nodeと同じExecution packetへ混在させず、A014のfragment jobは最大8に制限します。
