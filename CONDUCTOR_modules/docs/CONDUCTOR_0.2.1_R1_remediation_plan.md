...
| # | 項目 | 誰が決めるか |
|---|---|---|
| 1 | `k_min = 10` でよいか | **設計担当。** 「何件検出できれば設計が成立するか」は設計量である |
| 2 | `max_units` / memory 予算の具体値 | 実装者が本番機で測定して提案 |
| 3 | 進捗報告の間隔と stalled 判定時間 | 実装者が提案（本書の 30秒 / 10分 は初期値） |
| 4 | L1b にも M-3 相当の変更が要るか | **段階R1-9 の測定後** |
| 5 | Mordred 導入後の実特徴量数と候補化率 | 本番機で測定 |
