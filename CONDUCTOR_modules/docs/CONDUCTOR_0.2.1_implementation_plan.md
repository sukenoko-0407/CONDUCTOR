...
| 条件付け深度 | **1 のみ**（仕様書は最大2だが 0.2.1 では実装しない。段階9-3 参照） |
...
#### 段階9-3 深度2条件を実装しない（C-6 の決定）

仕様書では条件付け深度を最大2としていたが、**0.2.1 では深度1のみ実装する。**

理由は次の2点である。

- 較正データで深度1の文脈が 270 あり、重複排除後でも十分な数がある
- 深度2は多重比較の族が爆発する一方、有効性が未測定である

深度2は 0.2.2 以降で、診断により有効性を確認してから導入する。
...
| 項目 | 期待値 | 許容 |
|---|---|---|
| L2b の enrichment | 2.07 / 2.31 / 2.54 | ±30% |
| L5 の enrichment | 2.19 / 1.82 / 1.81 | ±30% |
| L1b の enrichment | 1.45 / 1.43 / 1.29 | ±30% |
| 変換3クラスの pair 数 | TERM 9,548 / RING 1,579 / LINK 436 | ±10% |
| 文脈数（重複排除前） | 270 | ±20% |
