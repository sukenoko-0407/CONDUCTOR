...
class BetaContracts(unittest.TestCase):
    def test_catalog_and_ids(self)->None:
        self.assertEqual("0.1.1",(MODULES/"VERSION").read_text(encoding="utf-8").strip())
        catalog=json.loads((MODULES/"catalog"/"catalog.json").read_text(encoding="utf-8"));self.assertEqual("0.1.1",catalog["conductor_version"])
