...
class Runtime012Tests(unittest.TestCase):
    ...
    def test_round_requires_human_authorization_and_uses_one_live_lease(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            returned = json.loads(self.command("verify-return", "--run-root", str(run_root), "--confirm-returned", "--control-key", control_key, "--owner-id", "test-session", "--start-revision", str(resumed["control_revision"])).stdout)
