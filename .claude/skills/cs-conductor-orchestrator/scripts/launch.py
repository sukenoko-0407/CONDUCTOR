...
cache = env_dir / "cache"
locations = {
    "PIXI_HOME": env_dir / "pixi-home", "PIXI_CACHE_DIR": cache / "pixi",
    ...
    "UV_CACHE_DIR": cache / "uv", "PIP_CACHE_DIR": cache / "pip",
    "XDG_CACHE_HOME": cache / "xdg", "XDG_CONFIG_HOME": env_dir / "config",
    "XDG_DATA_HOME": env_dir / "data", "XDG_STATE_HOME": env_dir / "state",
...
if not (ready.is_file() and ready.read_text(encoding="utf-8").strip() == lock_hash):
    ...
    for _ in range(600):
        try:
            mutex.mkdir(); acquired = True; break
