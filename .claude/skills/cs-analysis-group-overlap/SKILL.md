---
name: cs-analysis-group-overlap
description: Use when Claude Code needs to run Group overlap from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Group overlapを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。
...
`--membership`を必須とする。CONDUCTOR初手では重複MCS Groupを持ち得るC002とfragment GroupingのC003を評価する。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A009_group_overlap.csv`だけを生成する。
