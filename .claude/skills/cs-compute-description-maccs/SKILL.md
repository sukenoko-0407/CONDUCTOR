---
name: cs-compute-description-maccs
description: Use when Claude Code needs to run MACCS keys from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからMACCS keysを計算する。
...
固定長MACCS keysを計算する。algorithm固有optionはない。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D003_maccs.csv`（または`--format parquet`）だけを生成する。
