---
name: cs-compute-description-mordred-3d
description: Use when Claude Code needs to run Mordred 3D descriptors from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからMordred 3D descriptorsを計算する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D016_mordred_3d.csv`（または`--format parquet`）だけを生成する。
