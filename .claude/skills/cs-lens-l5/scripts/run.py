...
def _artifact(output: Path, role: str, name: str, rows: int | None) -> dict[str, Any]:
    ...
    return {"artifact_id": stable_id("ART", {"role": role, "sha256": digest}), "role": role, "path": name, "media_type": "application/json" if name.endswith(".json") else "application/x-ndjson" if name.endswith(".jsonl") else "text/csv", "schema": f"{role}@0.2.1", "rows": rows, "sha256": digest}
...
def execute(args: argparse.Namespace) -> dict[str, str]:
    ...
    if request["identity"]["skill_name"] != "cs-lens-l5" or request["parameters"].get("operation") != "l5":
        raise SchemaValidationError("Request must target cs-lens-l5 operation l5")
    ...
    result = run_l5(
        pd.read_csv(_input(request, "compounds"), dtype={"compound_id": "string"}),
    ...
    files = [("l5_evidence", "l5_evidence.csv", result.evidence), ("l5_tests", "l5_tests.csv", result.tests), ("score_observations", "score_observations.csv", result.score_observations)]
    for _, name, frame in files:
        _csv(frame, output / name)
    _jsonl(result.findings, output / "findings.jsonl")
    atomic_write_json(output / "l5_calibration.json", result.calibration)
    for finding in result.findings:
        validate_instance(finding, SCHEMAS / "finding.schema.json")
    status = "succeeded" if result.calibration["acceptance_gt_1_5"] and result.metrics["participation_rate"] >= float(config["statistics"]["min_permutation_participation"]) else "needs_design_review"
    artifacts = [_artifact(output, role, name, len(frame)) for role, name, frame in files]
    artifacts += [_artifact(output, "findings", "findings.jsonl", len(result.findings)), _artifact(output, "l5_calibration", "l5_calibration.json", None)]
    manifest = {"schema_version": "0.2.1", "producer": {key: request["identity"][key] for key in ("run_id", "node_id", "attempt_id", "skill_name")}, "status": status, "config_sha256": file_sha256(config_path), "input_artifacts": [{"role": item["role"], "path": item["path"], "sha256": item["sha256"]} for item in request["inputs"]], "artifacts": artifacts, "metrics": result.metrics, "warnings": [] if status == "succeeded" else ["L5 calibration or permutation participation requires design review"], "created_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")}
    atomic_write_json(output / "artifact_manifest.json", manifest)
    validate_instance(manifest, SCHEMAS / "artifact_manifest.schema.json")
    return {"status": status, "manifest": "artifact_manifest.json", "primary": "findings.jsonl"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CONDUCTOR 0.2.1 L5 lens")
    parser.add_argument("--request", required=True); parser.add_argument("--output-dir", required=True); parser.add_argument("--workers", type=int, default=0); parser.add_argument("--overwrite", action="store_true"); parser.add_argument("--estimate-work", action="store_true")
    ...
    try:
        ...
        print(json.dumps({"status": "failed", "error": str(exc)}), file=sys.stderr); return 3
    except Exception as exc:
        print(json.dumps({"status": "failed", "error": str(exc)}), file=sys.stderr); return 4
