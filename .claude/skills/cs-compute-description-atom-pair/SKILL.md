---
name: cs-compute-description-atom-pair
description: Use when Claude Code needs to run Hashed atom-pair fingerprint from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからHashed atom-pair fingerprintを計算する。
...
`--n-bits`でhashed count fingerprintの次元を指定する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D004_atom_pair.csv`（または`--format parquet`）だけを生成する。
