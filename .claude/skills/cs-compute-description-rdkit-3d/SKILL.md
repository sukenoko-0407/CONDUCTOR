---
name: cs-compute-description-rdkit-3d
description: Use when Claude Code needs to run RDKit 3D descriptors from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからRDKit 3D descriptorsを計算する。
...
`--num-confs`と`--random-seed`でconformer探索を制御する。入力SMILESから3D conformerを生成するため、2D手法より計算量が大きい。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D012_rdkit_3d.csv`（または`--format parquet`）だけを生成する。
