---
name: cs-compute-description-mordred-2d
description: Use when Claude Code needs to run Mordred 2D descriptors from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからMordred 2D descriptorsを計算する。
...
Mordred 2D descriptor集合を固定仕様で計算する。algorithm固有optionはない。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D015_mordred_2d.csv`（または`--format parquet`）だけを生成する。
