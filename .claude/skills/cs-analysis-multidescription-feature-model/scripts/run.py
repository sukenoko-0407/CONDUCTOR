...
def run()->int:
    ...
    config={k:v for k,v in vars(a).items() if k!="description_paths"}; manifest={"schema_version":"2.0.0","conductor_version":"0.1.3","artifact_stage":"analysis","run_id":a.run_id or "standalone","node_id":a.node_id,"attempt_id":a.attempt_id,"capability_id":CAPABILITY["capability_id"],"skill_name":CAPABILITY["skill_name"],"skill_version":CAPABILITY["version"],"input":a.input,"input_hash":input_hash,"value_semantics":"model_evaluation","natural_metric":None,"warnings":[],"created_at":now(),"configuration":config,"fixed_description_panel":list(PANEL)}
    if a.conductor:
