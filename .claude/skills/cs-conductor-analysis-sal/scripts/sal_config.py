...
def descriptor_dir(input_path: str | Path, config: dict[str, Any], explicit: str | None = None) -> Path:
    if explicit:
        return Path(explicit)
    description_cfg = config.get("description_inputs", {}) or {}
...
def default_outdir(input_path: str | Path, config: dict[str, Any], explicit: str | None = None) -> Path:
    ...
    if subdir:
        return base_dir / str(subdir)
    return base_dir / Path(input_path).stem / "structure_activity_landscape"


def match_representation(filename: str, config: dict[str, Any]) -> tuple[str, dict[str, Any]] | None:
    reps = config.get("representations", {}) or {}
    for representation_id, rep_cfg in reps.items():
        pattern = str((rep_cfg or {}).get("match", ""))
        if pattern and fnmatch.fnmatch(filename, pattern):
            return str(representation_id), dict(rep_cfg)
    return None
