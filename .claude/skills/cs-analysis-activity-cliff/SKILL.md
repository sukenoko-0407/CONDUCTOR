...
description: Use when Claude Code needs to run Activity cliff detection from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Activity cliff detectionを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。
...
`--similarity-threshold`、`--activity-delta-threshold`、`--max-pairs`を指定する。pair上限超過時は`--random-seed`（既定61453）による一様ランダム非復元抽出を行う。任意のGroup内または二Group間へscopeを限定し、CliffのGroup内部・境界局在を比較できる。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A007_activity_cliff.csv`だけを生成する。
