...
description: Use when Claude Code needs to run Categorical-column clustering from CSV or compatible CONDUCTOR artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Categorical-column clusteringを単独実行し、Clustering artifactを生成する。

## Input

compound IDと一つ以上のカテゴリ列を持つCSVを使い、`--columns assay,series`のようにClusteringへ使用する列を必ず指定する。 分子標準化、活性単位変換、pActivity変換は行わない。
...
`--columns`を必須とし、`--min-cluster-size`未満のClusterを登録しない。
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/launch.py" --input compounds.csv --columns assay --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id NC000001 --attempt-id ATT0001
```
