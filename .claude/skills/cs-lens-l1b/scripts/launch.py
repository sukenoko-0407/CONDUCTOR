...
def main() -> int:
    ...
    return subprocess.run([str(Path(pixi).resolve()), "run", "--manifest-path", str(skill / "env" / "pixi.toml"), "--locked", "python", str(skill / "scripts" / "run.py"), *sys.argv[1:]], env=env, check=False).returncode


if __name__ == "__main__":
    try: raise SystemExit(main())
    except Exception as exc:
        print(json.dumps({"status":"failed","error":str(exc)}), file=sys.stderr); raise SystemExit(4)
