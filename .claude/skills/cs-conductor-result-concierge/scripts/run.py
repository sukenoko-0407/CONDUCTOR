...
def validate_run_root(value: str) -> tuple[Path, Path, Path, dict[str, Any], dict[str, Any]]:
    ...
    if control.get("conductor_version") != "0.1.6" or not isinstance(snapshot.get("nodes"), list):
        raise ValueError("The supplied directory is not a CONDUCTOR 0.1.6 Run Root")
...
def parser() -> argparse.ArgumentParser:
    ...
    prepare_parser.add_argument("--run-root", required=True, help="Explicit path to a completed CONDUCTOR 0.1.6 Run Root.")
