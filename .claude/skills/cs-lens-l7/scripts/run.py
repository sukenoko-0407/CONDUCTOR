...
def estimate(args:argparse.Namespace)->WorkEstimate:
    request=json.loads(Path(args.request).resolve().read_text(encoding="utf-8")); validate_instance(request,SCHEMAS/"execution_request.schema.json")
    if request["identity"]["skill_name"]!="cs-lens-l7" or request["parameters"].get("operation")!="l7": raise SchemaValidationError("Request must target cs-lens-l7 operation l7")
    verify_request_inputs(request); config=yaml.safe_load(Path(request["config_path"]).resolve().read_text(encoding="utf-8")); return estimate_work(request,config,workers=max(1,int(args.workers)))
def main()->int:
    parser=argparse.ArgumentParser(description="CONDUCTOR 0.2.1 L7 lens"); parser.add_argument("--request",required=True); parser.add_argument("--output-dir",required=True); parser.add_argument("--workers",type=int,default=0); parser.add_argument("--overwrite",action="store_true"); parser.add_argument("--estimate-work",action="store_true"); args=parser.parse_args()
