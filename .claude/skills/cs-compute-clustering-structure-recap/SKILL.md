---
name: cs-compute-clustering-structure-recap
description: Group compounds from a compound-ID/SMILES CSV with RECAP fragment. Use for general clustering or an explicitly planned CONDUCTOR v4 structural Grouping node; this Skill requires CSV input and does not consume Description vectors or hide a fingerprint-generation step.
allowed-tools: Read, Write, Bash, Glob, Grep
---

# RECAP fragment clustering

## Purpose

SMILESをRECAP fragmentにより直接group化し、一般利用ではClustering、CONDUCTOR内ではGrouping artifactを生成する。
...
`--min-cluster-size`未満のfragment groupを出力しない。
