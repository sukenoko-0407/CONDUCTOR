...
GUIDANCE = {
    ...
    "cluster_enrichment": (
        "各Clusterに高活性化合物が濃縮されるかをeffect sizeと検定値から確認する。",
        "小Cluster、多重比較、全体と重なる大Clusterでは見かけの濃縮に注意する。",
    ),
    "cluster_overlap": (
        "異なるClustering由来Clusterの重複をJaccardと交差数で比較する。",
        "重複はClusterの独立性を示す情報であり、活性差や機序を単独では説明しない。",
    ),
    "cluster_structural_diversity": (
        "Cluster内部のMorgan/Tanimoto類似度から構造的な凝集度と多様性を確認する。",
        "構造凝集度はClusteringの有用性の一面であり、活性の均一性とは別に評価する。",
    ),
}
...
METRICS = {
    ...
    "cluster_enrichment": ["odds_ratio", "median_shift_vs_global"],
    "cluster_overlap": ["jaccard", "intersection_count"],
    "cluster_structural_diversity": ["structural_diversity_score", "mean_tanimoto"],
}
...
RANKING = {
    ...
    "cluster_enrichment": ("fisher_pvalue", True),
    "cluster_overlap": ("jaccard", False),
    "cluster_structural_diversity": ("structural_diversity_score", False),
}
...
def render_operator_report(
    ...
    scope = operator_summary.get("scope") or {}
    direction = "高い値が良好" if getattr(args, "higher_is_better", None) else "低い値が良好"
    description_context = operator_summary.get("evaluation_representation") or ("未指定（artifactあり）" if getattr(args, "description", None) else "未使用")
    clustering_context = operator_summary.get("clustering_representation") or ("未指定（artifactあり）" if getattr(args, "membership", None) else "未使用")
    description_node = getattr(args, "description_node_id", None) or "未記録"
    clustering_node = getattr(args, "clustering_node_id", None) or "未記録"
    target = scope.get("target_cluster_id") or "GLOBAL"
    comparison = scope.get("comparison_cluster_id") or "-"
    contexts = [
        ...
        ("Scope", f"{scope.get('mode', 'global')} · N={scope.get('sample_count', operator_summary.get('sample_count', 0))}"),
        ...
        ("Clustering source", f"{clustering_context} · Node {clustering_node}" if getattr(args, "membership", None) else str(clustering_context)),
    ...
    warning_items = list(operator_summary.get("limitations") or [])
    ...
    return f"""<!doctype html><html lang='ja'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{html.escape(capability['display_name'])} · CONDUCTOR Operator Report</title><style>{css}</style></head><body><main><header><p class='meta'>CONDUCTOR Operator report</p><h1>{html.escape(capability['display_name'])}</h1><p class='meta'>Run {html.escape(str(operator_summary.get('run_id') or '-'))} · Node {html.escape(str(getattr(args, 'node_id', None) or '-'))} · {html.escape(str(operator_summary.get('created_at') or '-'))}</p></header><h2>解析対象と由来</h2><dl class='context'>{context_html}</dl><div class='guidance'><div><b>この解析で確認すること</b><br>{html.escape(purpose)}</div><div class='caution'><b>解釈上の注意</b><br>{html.escape(caution)}</div></div><h2>主要結果</h2><p>{html.escape(str(operator_summary.get('headline') or ''))}</p><div class='metrics'>{cards}</div>{nested}<h2>結果分布</h2>{chart_html}<h2>個別結果</h2><p>解釈上確認しやすい順に最大100行を表示しています。完全な結果は<a href='{html.escape(result_path.name)}'>{html.escape(result_path.name)}</a>を参照してください。</p>{result_table(result, operator)}<h2>警告・制約</h2><div class='warning'><ul>{warning_html}</ul><p>{html.escape(caution)}</p></div><details><summary>実行parameterとprovenance</summary><table><tbody>{parameter_rows}</tbody></table><p class='trace'>Result ref: {html.escape(str(operator_summary.get('result_ref') or '-'))}<br>Input: {file_link(manifest.get('input'))}<br>Input hash: {html.escape(str(manifest.get('input_hash') or '-'))}</p></details></main></body></html>"""
