...
description: Use when Claude Code needs to run Vector connected-component clustering from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Vector connected-component clusteringを単独実行し、一般利用ではClustering、CONDUCTOR内ではGrouping artifactを生成する。
