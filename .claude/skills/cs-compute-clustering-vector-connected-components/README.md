...
Description Skillが生成した数値vectorへVector connected-component clusteringを適用し、cluster membershipとsummaryを生成する。
...
```bash
python .claude/skills/cs-compute-clustering-vector-connected-components/scripts/launch.py --input description.csv --input-representation D001 --value-semantics dense_continuous --metric euclidean
```
...
```bash
python .claude/skills/cs-compute-clustering-vector-connected-components/scripts/launch.py --input description.csv --input-representation D001 --description-result path/to/run_root/description/N000001/result.json --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id NODE_ID --attempt-id ATT0001
```
