---
name: cs-compute-clustering-structure-louvain
description: Use when Claude Code needs to run Structure Louvain clustering from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
allowed-tools: Read, Write, Bash, Glob, Grep
---

# Structure Louvain clustering

## Purpose

Structure Louvain clusteringを単独実行し、一般利用ではClustering、CONDUCTOR内ではGrouping artifactを生成する。
...
`--min-cluster-size`未満のgroupを出力しない。`--radius`と`--n-bits`を指定できる。`--similarity-threshold`、`--resolution`、`--random-seed`を指定する。
