---
name: cs-compute-description-chemberta-embedding
description: Use when Claude Code needs to run ChemBERTa-100M-MLM embedding from CSV or compatible CONDUCTOR artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからChemBERTa-100M-MLM embeddingを計算する。

## Input

`--input CSV`または反復可能な`--smiles`を使う。CSVではcompound ID列とSMILES列を推定するが、曖昧なら`--id-column`と`--smiles-column`を指定する。個別SMILESへIDを与える場合は`--compound-id`を同じ回数指定する。 ChemBERTa-100M-MLMのローカルmodel weightを`--model-dir`または`CONDUCTOR_CHEMBERTA_MODEL_DIR`で指定する。外部からmodelを自動downloadせずCPUで実行する。 分子標準化、活性単位変換、pActivity変換は行わない。
...
`--model-dir`、環境変数、Skill-local設定の順でChemBERTa-100M-MLM weightを解決する。CPU batch推論と非special-token mean poolingに固定し、外部modelを自動downloadしない。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D020_chemberta_embedding.csv`（または`--format parquet`）だけを生成する。
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/launch.py" --input compounds.csv --model-dir /shared/models/ChemBERTa-100M-MLM --run-id general-001
```
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/launch.py" --input compounds.csv --model-dir /shared/models/ChemBERTa-100M-MLM --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id ND000001 --attempt-id ATT0001
```
