...
def run() -> int:
    ...
    manifest={"schema_version":"2.0.0","conductor_version":"0.1.8","artifact_stage":"analysis","run_id":a.run_id or "standalone","node_id":a.node_id,"attempt_id":a.attempt_id,"capability_id":CAPABILITY["capability_id"],"skill_name":CAPABILITY["skill_name"],"skill_version":CAPABILITY["version"],"input":a.input,"input_hash":value_hash([sha(path) for path in input_files]),"value_semantics":"projection_coordinates","natural_metric":metric,"warnings":[],"created_at":now(),"configuration":vars(a)}
    if a.conductor:
