---
name: cs-analysis-projection-pca
description: Use when Claude Code needs to run PCA projection from CSV or compatible CONDUCTOR artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
PCA projectionを実行し、客観的な数値結果とCONDUCTOR Operator summaryを生成する。
...
`--role projection-fit|cluster-overlay`を使う。overlayはGlobal座標を再利用し、再fitしない。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A003_projection_pca.csv`と`projection.png`を生成する。State用summary、manifest、execution eventは生成しない。
