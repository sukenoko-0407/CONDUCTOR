...
Description Skillが生成した数値vectorへButina clusteringを適用し、cluster membershipとsummaryを生成する。
...
```bash
python .claude/skills/cs-compute-clustering-vector-butina/scripts/launch.py --input description.csv --conductor --project PROJECT --run-id RUN_ID --node-id NODE_ID
```
