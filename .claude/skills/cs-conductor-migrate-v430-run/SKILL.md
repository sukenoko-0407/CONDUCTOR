...
2. 人間へscan summary、含有/除外Node数、警告、source/targetを提示する。
3. 人間が明示承認した場合だけ `apply --approve` を実行する。
4. `verify` を実行する。失敗時は新run rootを解析に使わず、報告する。
5. 成功後、通常の `cs-conductor-orchestrator` Agentを新 `state.json` へbootstrapする。旧Interpretationは参照資料であり、新RoundでInterpretationを作り直す。
