---
name: cs-compute-description-rdkit-fragment
description: Use when Claude Code needs to run RDKit fragment counts from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
allowed-tools: Read, Write, Bash, Glob, Grep
---

# RDKit fragment counts

## Purpose

CSVまたは1件以上のSMILESからRDKit fragment countsを計算する。
...
RDKit fragment count集合を固定仕様で計算する。algorithm固有optionはない。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D006_rdkit_fragment.csv`（または`--format parquet`）だけを生成する。
