...
description: Use when Claude Code needs to run kNN activity consistency from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
kNN activity consistencyを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A005_knn_activity_consistency.csv`だけを生成する。
