---
name: cs-analysis-knn-activity-consistency
description: Use when Claude Code needs to run kNN activity consistency from CSV or compatible CONDUCTOR artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
kNN activity consistencyを実行し、客観的な数値結果とCONDUCTOR Operator summaryを生成する。
...
`--description`と`--k`を指定する。metricはDescription manifestのsemanticsから拘束する。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A007_knn_activity_consistency.csv`を生成する。State用summary、manifest、execution eventは生成しない。
