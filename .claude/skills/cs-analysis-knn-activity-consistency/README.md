...
kNN activity consistencyを実行し、一般利用向け数値結果とCONDUCTOR向けOperator resultを生成する。
...
```bash
python .claude/skills/cs-analysis-knn-activity-consistency/scripts/launch.py --input compounds.csv --property-column pIC50 --higher-is-better --description description.csv --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id NODE_ID --attempt-id ATT0001
```
