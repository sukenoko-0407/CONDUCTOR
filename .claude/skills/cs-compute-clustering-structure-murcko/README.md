...
compound IDとSMILESからMurcko scaffold clusteringを実行し、cluster membershipとsummaryを生成する。
...
```bash
python .claude/skills/cs-compute-clustering-structure-murcko/scripts/launch.py --input compounds.csv --conductor --project PROJECT --run-id RUN_ID --node-id NODE_ID
```
