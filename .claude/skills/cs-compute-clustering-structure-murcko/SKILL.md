...
description: Cluster compounds from a compound-ID/SMILES CSV with Murcko scaffold clustering, without generating a hidden descriptor vector. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
compound IDとSMILESを含むCSVをMurcko scaffold clusteringにより直接Cluster化し、Clustering artifactを生成する。
...
`--min-cluster-size`未満のscaffold Clusterを登録しない。
