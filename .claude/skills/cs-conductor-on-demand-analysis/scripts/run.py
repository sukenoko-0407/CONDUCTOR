...
def frozen_state_errors(control: dict[str, Any], snapshot: dict[str, Any]) -> list[str]:
    # On-demand is outside Round control and reads a point-in-time copy.  An
    # ACTIVE Round or live lease is therefore not a refusal condition.
    return []
...
def validate_run_root(value: str) -> tuple[Path, Path, Path, dict[str, Any], dict[str, Any]]:
    ...
    snapshot_path = run_root / "runtime" / "dag.json"
    transaction_path = run_root / "runtime" / "state_transaction.json"
    if not control_path.is_file() or not snapshot_path.is_file():
        raise FileNotFoundError("conductor_control.json or runtime/dag.json is missing")
    if transaction_path.is_file():
        raise RuntimeError(
            "Run state has an unfinished Runtime transaction. Resume/recover the same Round before creating an On-demand snapshot."
        )
    ...
    if errors:
        raise RuntimeError("On-demand request refused: " + "; ".join(errors))
...
def candidate_sources(control_path: Path, snapshot_path: Path, snapshot: dict[str, Any], run_root: Path) -> list[Path]:
    ...
    for node in nodes:
        ...
        for value in (result.get("primary_path"), result.get("execution_event")):
            if isinstance(value, str):
                path = path_from_state(value, run_root)
                if path is not None and path.is_file(): candidates.append(path)
        output_dir = path_from_state(str(result.get("result_dir", "")), run_root)
        if output_dir is not None and output_dir.is_dir():
            for name in ("operator_summary.json", "operator_report.html", "standard_summary.html", "interpretation.json", "interpretation.md", "interpretation.html"):
                path = output_dir / name
                if path.is_file(): candidates.append(path.resolve())
    unique: dict[str, Path] = {}
    concierge_root = (run_root / "on_demand").resolve()
    ...
    interpretations = [
        {
            ...
            "output_dir": (node.get("result") or {}).get("result_dir"),
...
def run_inventory(run_root: Path) -> list[dict[str, Any]]:
    """Capture cheap mutation evidence for every run file outside on_demand/."""
    concierge_root = (run_root / "on_demand").resolve()
...
def resolve_request_dir(value: str) -> tuple[Path, Path, dict[str, Any]]:
    ...
    if not request_dir.is_dir() or not REQUEST_PATTERN.match(request_dir.name):
        raise ValueError(f"Invalid On-demand request directory: {request_dir}")
    ...
    expected_root = (run_root / "on_demand").resolve()
...
def prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (run_root / "on_demand").resolve() == run_root:
    ...
    draft = {
        ...
        "focus_ids": focus_ids,
        "source_paths": ["conductor_control.json", "runtime/dag.json"],
        "figures": [],
        "limitations": [],
...
def run_helper(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if request.get("status") not in {"prepared", "completed"}:
        raise ValueError(f"Unsupported On-demand request status: {request.get('status')}")
...
def project_root() -> Path:
    ...
    for candidate in here.parents:
        if (candidate/".claude"/"skills"/"cs-analysis-matched-molecular-pairs"/"scripts"/"launch.py").is_file() and (candidate/"CONDUCTOR_modules").is_dir():
            return candidate
...
def run_mmp(args: argparse.Namespace) -> dict[str, Any]:
    ...
    generated_path=request_dir/"generated_artifacts.json"
    previous=[]
...
def resolve_source(value: str, run_root: Path) -> Path:
    ...
    if not is_relative_to(source, run_root) or is_relative_to(source, concierge_root):
        raise ValueError("Source must be a read-only file under run_root and outside on_demand/")
    return source
...
def verify_source_manifest(request_dir: Path, run_root: Path) -> list[dict[str, Any]]:
    ...
    for entry in manifest.get("sources") or []:
        if entry.get("kind") == "runtime":
            # Live control/DAG may advance while this isolated request is open;
            # the request-local copies are the reproducible state reference.
            continue
        ...
        if not is_relative_to(path, run_root) or is_relative_to(path, (run_root / "on_demand").resolve()):
...
def markdown_report(request: dict[str, Any], draft: dict[str, Any], figures: list[dict[str, Any]]) -> str:
    ...
    lines.extend(["", "---", "このreportは通常Round/DAGから独立したREQ記録であり、既存の正規解析成果物を変更していない。", ""])
    return "\n".join(lines)
...
<body><main><header><div class="meta">CONDUCTOR On-demand Analysis · {html.escape(request['request_id'])} · {html.escape(request['finalized_at'])}</div><h1>{html.escape(draft['title'])}</h1><div>{focus}</div></header>
...
<div class="freeze">このreportは通常Round/DAGから独立したREQ記録であり、既存の正規解析成果物を変更していません。</div></main></body></html>
"""
...
def finalize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    generated_path=request_dir/"generated_artifacts.json"
    if generated_path.is_file(): captured.update(load_json(generated_path).get("artifacts") or [])
    ...
    atomic_write_text(request_dir / "result.md", markdown)
    atomic_write_text(request_dir / "result.html", html_text)
    if draft["suggested_next_round_prompt"]:
    ...
    output_paths = [request_dir / "result.md", request_dir / "result.html"]
    ...
    if generated_path.is_file():
        output_paths.extend(path for path in (request_dir/"artifacts").rglob("*") if path.is_file())
    request["outputs"] = {
    ...
    write_json(request_dir / "request.json", request)
    artifact_manifest = {
        "schema_version": "1.0.0", "conductor_version": VERSION,
        "request_id": request_dir.name, "human_request": request.get("human_request"),
        "source_manifest": "source_manifest.json", "outputs": request["outputs"],
        "round_participation": False, "canonical_dag_mutated": False,
        "created_at": request.get("created_at"), "finalized_at": request.get("finalized_at"),
    }
    write_json(request_dir / "artifact_manifest.json", artifact_manifest)
    index_path = run_root / "on_demand" / "index.jsonl"
    with index_path.open("a", encoding="utf-8", newline="\n") as handle:
        handle.write(json.dumps({"request_id": request_dir.name, "status": "completed", "request_dir": request_dir.name, "human_request": request.get("human_request"), "finalized_at": request.get("finalized_at")}, ensure_ascii=False) + "\n")
    return {
        "status": "completed",
        "request_id": request_dir.name,
        "response_markdown": str(request_dir / "result.md"),
        "response_html": str(request_dir / "result.html"),
...
def parser() -> argparse.ArgumentParser:
    ...
    prepare_parser = subparsers.add_parser("prepare", help="Create an isolated On-demand request workspace.")
    ...
    verify_parser = subparsers.add_parser("verify", help="Verify a finalized On-demand request and report later source drift.")
