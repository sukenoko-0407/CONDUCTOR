...
Group structural diversityを実行し、一般利用向け数値結果とCONDUCTOR向けevidenceを生成する。

## 想定利用シーン

各group内の構造的な多様性を評価する場合。
...
```bash
python .claude/skills/cs-analysis-group-structural-diversity/scripts/launch.py --input compounds.csv --property-column pIC50 --higher-is-better --membership cluster_membership.csv
```
...
```bash
python .claude/skills/cs-analysis-group-structural-diversity/scripts/launch.py --input compounds.csv --property-column pIC50 --higher-is-better --membership cluster_membership.csv --conductor --project PROJECT --run-id RUN_ID --node-id NODE_ID --round-id ROUND_ID --evidence-id EVIDENCE_ID
```
