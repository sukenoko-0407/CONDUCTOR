...
Pairwise structure similarityを実行し、一般利用向け数値結果とCONDUCTOR向けevidenceを生成する。
...
```bash
python .claude/skills/cs-analysis-pairwise-structure-similarity/scripts/launch.py --input compounds.csv --property-column pIC50 --higher-is-better --conductor --project PROJECT --run-id RUN_ID --node-id NODE_ID --round-id ROUND_ID --evidence-id EVIDENCE_ID
```
