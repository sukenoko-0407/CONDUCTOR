...
description: Use when Claude Code needs to run Pairwise structure similarity from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Pairwise structure similarityを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。
...
SMILES列を使い、`--max-pairs`でpair数を制限する。上限超過時は`--random-seed`（既定61453）による一様ランダム非復元抽出を行う。任意の`--membership --target-group --scope-mode within-group`でGroup内へ限定できる。`between-groups`では`--comparison-group`も指定し、Group間pairだけを評価する。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A003_pairwise_structure_similarity.csv`だけを生成する。
