...
```bash
python .claude/skills/cs-compute-clustering-vector-leiden/scripts/launch.py --input description.csv --input-representation D001
```
...
```bash
python .claude/skills/cs-compute-clustering-vector-leiden/scripts/launch.py --input description.csv --input-representation D001 --description-manifest path/to/description_manifest.json --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id NODE_ID --attempt-id ATT0001
```
