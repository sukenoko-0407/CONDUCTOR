---
name: cs-compute-description-morgan
description: Use when Claude Code needs to run Morgan fingerprint (optional chirality) from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
allowed-tools: Read, Write, Bash, Glob, Grep
---

# Morgan fingerprint (optional chirality)

## Purpose

CSVまたは1件以上のSMILESからMorgan fingerprint (optional chirality)を計算する。
...
`--radius`、`--n-bits`、`--encoding bit|count`、`--use-features`を指定できる。chiralityを含めない`--no-include-chirality`がdefaultで、chiralityを含める場合だけ`--include-chirality`を指定する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D002_morgan.csv`（または`--format parquet`）だけを生成する。
