...
description: Cluster compounds from a compound-ID/SMILES CSV with BRICS fragment clustering, without generating a hidden descriptor vector. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
compound IDとSMILESを含むCSVをBRICS fragment clusteringにより直接Cluster化し、Clustering artifactを生成する。
