---
name: cs-compute-clustering-structure-brics
description: Group compounds from a compound-ID/SMILES CSV with BRICS fragment. Use for general clustering or an explicitly planned CONDUCTOR v4 structural Grouping node; this Skill requires CSV input and does not consume Description vectors or hide a fingerprint-generation step.
...
SMILESをBRICS fragmentにより直接group化し、一般利用ではClustering、CONDUCTOR内ではGrouping artifactを生成する。
