---
name: cs-compute-description-shape
description: Use when Claude Code needs to run Basic 3D shape descriptors from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
allowed-tools: Read, Write, Bash, Glob, Grep
---

# Basic 3D shape descriptors

## Purpose

CSVまたは1件以上のSMILESからBasic 3D shape descriptorsを計算する。
...
`--num-confs`と`--random-seed`でconformer探索を制御する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D014_shape.csv`（または`--format parquet`）だけを生成する。
