...
description: Use when Claude Code needs to run Overlap-based meta clustering from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Overlap-based meta clusteringを単独実行し、一般利用ではClustering、CONDUCTOR内ではGrouping artifactを生成する。
...
`--similarity-threshold`でsource group間Jaccard graphのedgeを定義し、`--min-cluster-size`未満の統合groupを出力しない。
