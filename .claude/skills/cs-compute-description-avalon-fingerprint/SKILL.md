---
name: cs-compute-description-avalon-fingerprint
description: Use when Claude Code needs to run Avalon fingerprint from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからAvalon fingerprintを計算する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D010_avalon.csv`（または`--format parquet`）だけを生成する。
