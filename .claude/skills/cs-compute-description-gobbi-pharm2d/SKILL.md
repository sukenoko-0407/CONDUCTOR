---
name: cs-compute-description-gobbi-pharm2d
description: Use when Claude Code needs to run Gobbi 2D pharmacophore fingerprint (optional SVD) from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからGobbi 2D pharmacophore fingerprint (optional SVD)を計算する。
...
folded fingerprintは`--reduction none --n-bits N`、dataset単位の低次元表現は`--reduction svd --svd-dim N`を使う。SVDはvalid moleculeが2件以上必要で、`--random-seed`を記録する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D017_gobbi_pharm2d.csv`（または`--format parquet`）だけを生成する。
