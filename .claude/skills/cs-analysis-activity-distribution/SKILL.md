...
description: Use when Claude Code needs to run Activity distribution from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Activity distributionを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。
...
全体分布をdefaultとし、任意の`--membership`と`--target-group`でgroup別に限定できる。
