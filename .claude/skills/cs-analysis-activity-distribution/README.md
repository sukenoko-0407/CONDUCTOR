...
Activity distributionを実行し、一般利用向け数値結果とCONDUCTOR向けOperator resultを生成する。

## 想定利用シーン

endpoint全体または指定Clusterの分布を最初に把握する場合。
...
```bash
python .claude/skills/cs-analysis-activity-distribution/scripts/launch.py --input compounds.csv --property-column pIC50 --higher-is-better --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id NODE_ID --attempt-id ATT0001
```
