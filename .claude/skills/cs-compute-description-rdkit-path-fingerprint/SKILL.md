---
name: cs-compute-description-rdkit-path-fingerprint
description: Use when Claude Code needs to run RDKit path fingerprint from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからRDKit path fingerprintを計算する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D007_rdkit_path.csv`（または`--format parquet`）だけを生成する。
