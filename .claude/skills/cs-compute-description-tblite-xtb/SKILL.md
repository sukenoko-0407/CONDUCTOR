---
name: cs-compute-description-tblite-xtb
description: Use when Claude Code needs to run GFN2-xTB quantum descriptors from CSV or compatible CONDUCTOR artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからGFN2-xTB quantum descriptorsを計算する。
...
Linuxでは各workerを`spawn`で起動し、Scheduler/cpusetが許可したCPU集合の中から最大4 CPUを重複なく割り当てる。OpenMP設定はNumPy・tbliteのimport前に確定し、nested OpenMPを無効化する。OpenBLASは1 threadに抑え、worker内での二重並列を避ける。`description_manifest.json`の`metadata`にCPU予算、affinity、thread環境、worker観測値を記録する。
...
Runtime経由ではSkillのCONDUCTOR出力はattempt scratchとして検証され、成功時に0.1.4の最小正本artifactへ昇格される。

## Output contract

- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D019_tblite_xtb.csv`（または`--format parquet`）だけを生成する。
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/launch.py" --input compounds.csv --run-id general-001 --compound-workers 2 --cores-per-compound 4 --available-cpu-cores 8
```
