...
description: Use when Claude Code needs to run GFN2-xTB single-point descriptors from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
CSVまたは1件以上のSMILESからGFN2-xTB single-point descriptorsを計算する。
...
- 通常モード: `results/description/<input>/<skill>/<run-id>/`へ`D020_tblite_xtb.csv`（または`--format parquet`）だけを生成する。
