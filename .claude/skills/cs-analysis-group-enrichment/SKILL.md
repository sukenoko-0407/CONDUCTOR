---
name: cs-analysis-group-enrichment
description: Use when Claude Code needs to run Group activity enrichment from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
allowed-tools: Read, Write, Bash, Glob, Grep
---

# Group activity enrichment

## Purpose

Group activity enrichmentを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。

## Input

元CSV、endpoint列、`--higher-is-better`または`--no-higher-is-better`を必ず指定する。 `--membership`でGrouping membershipを必ず指定する。 分子標準化、活性単位変換、pActivity変換は行わない。
...
`--membership`を必須とし、`--high-quantile`、`--low-quantile`を指定する。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A008_group_enrichment.csv`だけを生成する。
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/launch.py" --input compounds.csv --property-column pIC50 --higher-is-better --membership path/to/cluster_membership.csv --conductor --project PROJECT --run-id RUN_ID --node-id NODE_ID --round-id ROUND_ID --evidence-id EVIDENCE_ID
```
