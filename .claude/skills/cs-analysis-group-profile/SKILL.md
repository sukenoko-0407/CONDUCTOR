---
name: cs-analysis-group-profile
description: Use when Claude Code needs to run Group profile from CSV or compatible CONDUCTOR v4 artifacts with a self-contained Pixi environment. General mode is the default; use CONDUCTOR mode only as an explicit opt-in with complete project, run, and node context.
...
Group profileを実行し、客観的な数値結果とCONDUCTOR evidenceを生成する。
...
`--membership`を必須とし、`--high-quantile`、`--low-quantile`、任意の`--target-group`を指定できる。
...
- 通常モード: `results/analysis/<input>/<skill>/<run-id>/`へ`A001_group_profile.csv`だけを生成する。
