...
Description Skillが生成した数値vectorへVector Louvain clusteringを適用し、cluster membershipとsummaryを生成する。
...
```bash
python .claude/skills/cs-compute-clustering-vector-louvain/scripts/launch.py --input description.csv
```
...
```bash
python .claude/skills/cs-compute-clustering-vector-louvain/scripts/launch.py --input description.csv --conductor --project PROJECT --run-id RUN_ID --round-id RND0001 --node-id NODE_ID --attempt-id ATT0001
```
