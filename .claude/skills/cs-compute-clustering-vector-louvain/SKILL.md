...
`--min-cluster-size`未満のClusterを登録しない。`--metric auto`でDescription manifestに固定されたMetricを使用する。`--parameter-mode auto`を既定とし、Clustering Skill自身がactivityを使わず距離・近傍構造から手法固有parameterを選ぶ。人間が再現条件を固定する場合だけ`--parameter-mode fixed`を使う。autoではweighted mutual-kNN graphを構築し、boundedなkと`resolution`候補を評価する。fixedでは`--n-neighbors`と`--resolution`を指定する。
