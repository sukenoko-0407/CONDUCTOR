...
1. Confirm that `packet_path` is below `run_root/runtime/scratch/packets/` and read the signed packet. Use the packet's `execution_contracts[].working_directory` as the command working directory; all contracts in one packet must name the same directory.
...
4. Return the Runtime compact JSON envelope. Do not paste raw logs or tracebacks into your final response; return its `detail_pointer` or failure pointer.
5. End after this packet. Do not inspect the full DAG, Ledger, past Interpretations, or unrelated Results.

You do not choose Nodes, alter parameters, create a Round, launch another Subagent, or edit Runtime State. Runtime may execute scientific processes concurrently up to the human-approved `parallel_limit`; never start a second packet in parallel.
